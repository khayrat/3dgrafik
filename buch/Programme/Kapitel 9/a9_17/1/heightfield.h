...
void heightfield::seamless_plasma( long g )
{
  ...
  long seed = 1185275600;

  srand( seed );  divide( 0, 0, xscale-1, 0, 0 );
  srand( seed );  divide( 0, yscale-1, xscale-1, yscale-1, 0 );

  seed = 592637800;

  srand( seed );  divide( 0, 0, 0, yscale-1, 0 );
  srand( seed );  divide( xscale-1, 0, xscale-1, yscale-1, 0 );
