...
void texture::load( long rcount, heightfield *hf )
{
  ...
  for( long y=0 ; y<surface.yscale ; y++ )
     for( long x=0 ; x<surface.xscale ; x++ )
     {
       ...
       pixel_32 c = bmp.bitmaps[ h ].picture[ offset ];
       double t = (detail.picture[ offset ].blue / 255.0);
       c.red = uchar( t * c.red + 0.5 );  c.green = uchar( t * c.green + 0.5 );  c.blue = uchar( t * c.blue + 0.5 );

       surface.picture[ y * surface.xscale + x ] = c;
     }
