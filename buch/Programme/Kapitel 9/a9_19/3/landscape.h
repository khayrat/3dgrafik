...
void landscape::load( long dl )
{
  ...
  for( y=0 ; y<detail_level ; y++ )
     for( x=0 ; x<detail_level ; x++ )
	 {
       ...
       vs[ y * detail_level + x ].tx = x * (1.0 / (detail_level-1));
       vs[ y * detail_level + x ].ty = y * (1.0 / (detail_level-1));
	 }
...
void landscape::load_heights( void )
{
  ...
  shadow_map.shadow( &terrain );

  for( long x=0 ; x<vertex_count ; x++ )
  {
    vs[ x ].wy = 0.2 * terrain.heights[ x ];
    vs[ x ].color = pixel_32( 255, 255, 255 );
    if( shadow_map.picture[ x ].blue == 0 ) vs[ x ].color = pixel_32( 150, 150, 150 );
  }
}
