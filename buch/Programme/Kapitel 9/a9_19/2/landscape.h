...
void landscape::load_heights( void )
{
  ...
  {
    vs[ x ].wy = 0.1 * terrain.heights[ x ];

    pixel_32 c = hc.components[ terrain.heights[ x ] ];
    double t = 1;
    if( shadow_map.picture[ x ].blue == 0 ) t = 0.5;

    vs[ x ].color = pixel_32( uchar( t * c.red + 0.5 ), uchar( t * c.green + 0.5 ), uchar( t * c.blue + 0.5 ) );
  }
}
