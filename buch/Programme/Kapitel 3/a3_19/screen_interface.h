...
class hardware_interface
{
  ...
  public:
    ...
    void draw_points( long point_count, hvertex *vs );
    void line_loop( long point_count, hvertex *vs );
    void line_strip( long point_count, hvertex *vs );
...
void hardware_interface::draw_points( long point_count, hvertex *vs )
{
  hvertex *buffer;
  ...
  if( FAILED( rendering_device->BeginScene() ) ) return;
    ...
    rendering_device->DrawPrimitive( D3DPT_POINTLIST, 0, point_count );
