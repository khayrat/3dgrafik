...
class polyeder
{
  ...
  public:
    ...
    polyeder( const polyeder &p ) {  exit_error( "polyeder: Aufruf Kopierkonstruktor\n" );  }
    polyeder( char *filename );
    polyeder( void ) : vertex_count( 0 ), polygon_count( 0 ), vs( NULL ), ps( NULL ) { }
   ~polyeder( void ) {  if( vs ) delete [] vs;  if( ps ) delete [] ps;  }
};
...
void polyeder::display( pixel_32 *sbuffer )
{
  for( long x=0 ; x<polygon_count ; x++ ) ps[ x ].display( vs, sbuffer );
}

void polyeder::load( char *filename )
{
  FILE *input;

  if( (input = fopen( filename, "rb" )) == NULL )
    exit_nofile( "polyeder::polyeder()", filename );
