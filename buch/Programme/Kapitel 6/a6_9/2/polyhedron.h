...
class polyhedron
{
  ...
  public:
    ...
    void set_unit_length( double (*get_length) ( vertex v ) );

    void plane( long dl );
    void sphere( long dl );
    void cylinder( long dl, double radius );
    void torus( long dl, double radius );
    void cone( long dl, double radius, double height );
  ...
  friend class floating_polyhedron;
};

void polyhedron::set_unit_length( double (*get_length) ( vertex v ) )
{
  for( long x=0 ; x<vertex_count ; x++ )
  {
    double length = get_length( vs[ x ] );
...
void polyhedron::load( polyhedron *p )
{
  ...
  for( x=0 ; x<vertex_count ; x++ ) vs[ x ] = p->vs[ x ];

  polygon_count = p->polygon_count;

  if( (ps = new polygon[ polygon_count ]) == NULL )
    exit_error( "*ps: Fehler bei der Reservierung von Arbeitsspeicher.\n" );

  for( x=0 ; x<polygon_count ; x++ )
  {
    ps[ x ].point_count = p->ps[ x ].point_count;

    if( (ps[ x ].points = new long[ ps[ x ].point_count ]) == NULL )
      exit_error( "*ps[ x ].points: Fehler bei der Reservierung von Arbeitsspeicher.\n" );
    
    for( long y=0 ; y<ps[ x ].point_count ; y++ )
       ps[ x ].points[ y ] = p->ps[ x ].points[ y ];

    ps[ x ].color_offset = p->ps[ x ].color_offset;
    ps[ x ].normal = p->ps[ x ].normal;
  }
}
...
void polyhedron::cone( long dl, double radius, double height )
{
  initialise_vertices( dl );

  double t = radius / height;

  for( long y=0 ; y<detail_level ; y++ )
     vs[ y*detail_level ] = vertex( -y*t, -y, 0 );
