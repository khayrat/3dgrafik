...
#define render_mode D3DFVF_XYZ | D3DFVF_NORMAL
...
class hardware_interface
{
  ...
  public:
    ...
    void set_material( D3DMATERIAL9 *material ) {  rendering_device->SetMaterial( material );  }
...
{
  rendering_device->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB( 0, 0, 0 ), 1, 0 );
}
...
void hardware_interface::draw_polygon( long point_count, hvertex *vs )
{
  ...
  if( FAILED( rendering_device->BeginScene() ) ) return;
    ...
    rendering_device->SetStreamSource( 0, vertex_buffer, 0, sizeof( hvertex ) );
    rendering_device->SetFVF( render_mode );
    rendering_device->DrawPrimitive( D3DPT_TRIANGLELIST, 0, point_count/3 );
...
void hardware_interface::initialise_platform( void )
{
  ...
  D3DLIGHT9 light_source;

  memset( &light_source, 0, sizeof( D3DLIGHT9 ) );
  light_source.Type = D3DLIGHT_DIRECTIONAL;
  light_source.Diffuse.r = 1.0f;
  light_source.Diffuse.g = 1.0f;
  light_source.Diffuse.b = 1.0f;

  D3DXVECTOR3 light_direction( -1, -1, 1 );
  D3DXVec3Normalize( (D3DXVECTOR3*) &light_source.Direction, &light_direction );

  rendering_device->SetLight( 0, &light_source );
  rendering_device->LightEnable( 0, TRUE );
  rendering_device->SetRenderState( D3DRS_LIGHTING, TRUE );
  rendering_device->SetRenderState( D3DRS_SHADEMODE, D3DSHADE_PHONG );
  rendering_device->SetRenderState( D3DRS_AMBIENT, 0xffffffff );
}
