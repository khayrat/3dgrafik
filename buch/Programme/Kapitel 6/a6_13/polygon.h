...
class polygon
{
  private:
    ...
    void rasterize( pixel_32 *sbuffer, pixel_32 color );
...
void polygon::project( void )
{
  for( long x=0 ; x<cp_point_count ; x++ )
  {
    ...
    spoint[ x ].sx = long( wpoint[ x ].wx / wpoint[ x ].wz *  800 + x_res/2 );
    spoint[ x ].sy = long( wpoint[ x ].wy / wpoint[ x ].wz * -800 + y_res/2 );
...
double get_color( vector normal )
{
  ...
  if( cos_alpha < 0.0 )
    return pow( mt( acos( cos_alpha ) ), 6 ) * (used_colors.range_size - 1);
...
void polygon::rasterize( pixel_32 *sbuffer, pixel_32 color )
{
