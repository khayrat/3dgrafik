...
class bitmap_32
{
  public:
    ...
    void display_partial( long px, long py, long x_length, long y_length, long offset, pixel_32 *sbuffer );
...
void bitmap_32::display_partial( long px, long py, long x_length, long y_length, long offset, pixel_32 *sbuffer )
{
  long x, y, partial_offset;
  
  partial_offset = py * xscale + px;
  for( y=0 ; y<y_length ; y++ )
  {
    for( x=0 ; x<x_length ; x++ ) sbuffer[ offset++ ] = picture[ partial_offset++ ];

    offset += x_res - x_length;
    partial_offset += xscale - x_length;
  }	
}
