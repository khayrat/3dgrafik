...
double xh( double x )
{
  return 5*sin( x+add )*sin( x+add )*sin( x+add ) + 40*sin( x-add )*sin( x-add ) + 100*sin( x+add );
}

double yh( double y )
{
  return 80*sin( y-add ) + 15*sin( y )*sin( y )*sin( y ) - 20*sin( y-add )*sin( y );
}
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow )
{
  ...
  while( 1 )
  {
    ...
    for( long y=0 ; y<yscale ; y++ ) y_values[ y ] = yh( y*t );

    for( long x=0 ; x<xscale ; x++ )
    {
      double x_value = xh( x*t );
