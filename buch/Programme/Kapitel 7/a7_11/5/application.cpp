...
void draw_lens( uchar *picture, pixel_32 *pal, pixel_32 *sbuffer )
{
  ...
  for( short y = -r ; y <= r ; y++ )
  {
    ...
    for( short x = -b ; x <= b ; x++ )
    {
      double alpha = get_angle( sqrt( x*x+y*y ) );
      long dx = long( x * cos( alpha ) - y * sin( alpha ) ) + lens_pos.sx;
      long dy = long( x * sin( alpha ) + y * cos( alpha ) ) + lens_pos.sy;

      sbuffer[ offset + x ] = pal[ picture[ dy * x_res + dx ] ];
    }
