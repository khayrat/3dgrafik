...
double f( double x )
{
  return (1.5 / (lens_radius * lens_radius))* x*x + 0.5;
}
...
double g( double x )
{
  ...
  return 1.5/pow( lens_radius, n ) * pow( x, n ) + 0.5;
}
...
void draw_lens( uchar *picture, pixel_32 *pal, pixel_32 *sbuffer )
{
  ...
  for( short y = -r ; y <= r ; y++ )
  {
    ...
    for( short x = -b ; x <= b ; x++ )
    {
      double t = f( sqrt( x*x+y*y ) );

      long dx = long( t * x ) + lens_pos.sx;
      long dy = long( t * y ) + lens_pos.sy;
