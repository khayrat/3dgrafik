...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow )
{
  ...
  message( "Steuerung:", "Pfeiltasten:\tAuswahl eines Gebietes\nEnter:\t\tVergrößerung des ausgewählten Bereiches\n\nEscape:\t\tProgramm beenden" );
  ...
  while( 1 )
  {
    ...
    if( redraw_fractal == 1 )
    {
      long double bcx = (end_cx - begin_cx) / double( xscale ) * lens.pos.sx + begin_cx;
      long double ecx = (end_cx - begin_cx) / double( xscale ) * (lens.pos.sx+lens.size) + begin_cx;

      long double bcy = (begin_cy - end_cy) / double( yscale ) * (lens.pos.sy+lens.size) + end_cy;
      long double ecy = (begin_cy - end_cy) / double( yscale ) * lens.pos.sy + end_cy;

      begin_cx = bcx;  end_cx = ecx;  begin_cy = bcy;  end_cy = ecy;
