...
void polygon::rasterize( pixel_32 *sbuffer )
{
  ...
  { 
    ...
    while( length-- > 0 )
    {
      ...
      {
      	double z = act_z - clear_translation;
      	
      	pixel_32 color;
      	
      	if( z <= 12 ) color = pixel_32( 255, 0, 0 );
        else if( z > 12 && z <= 14 ) color = pixel_32( 0, 255, 0 );
      	else if( z > 14 && z <= 16 ) color = pixel_32( 0, 0, 255 );
      	else if( z > 16 && z <= 18 ) color = pixel_32( 255, 255, 0 );
      	else if( z > 18 && z <= 20 ) color = pixel_32( 255, 0, 255 );
      	else if( z > 20 ) color = pixel_32( 0, 255, 255 );
...
void polygon::shape( pixel_32 *sbuffer )
{
  ...
  for( x=cp_point_count-1, y=0 ; y<cp_point_count ; x=y, y++ )
     draw_line( spoint[ x ], spoint[ y ], pixel_32( 255, 255, 255 ), sbuffer );
}
