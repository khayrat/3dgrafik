...
void polygon::load( FILE *input, vertex *vs )
{
  ...
  if( point_count < 3 || point_count >= largest_point_count )
    exit_error( "polygon::load(): Der Wert von point_count ist ungültig.\n" );
  ...
  if( color_offset < 0 || color_offset >= used_colors.color_count )
    exit_error( "polygon::load(): Der Wert von color_offset ist ungültig.\n" );
