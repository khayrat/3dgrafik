...
svertex get_xplane_coords( svertex a, svertex b, long plane_x )
{
  ...
  c.tx = m * (a.tx - b.tx) + b.tx;
  c.ty = m * (a.ty - b.ty) + b.ty;
  c.inv_z = m * (a.inv_z - b.inv_z) + b.inv_z;
...
void polygon::rasterize( pixel_32 *sbuffer )
{
  ...
  { 
    ...
    invz_step = (right_side[ y ].inv_z - left_side[ y ].inv_z) / length;
    ...
    while( length-- > 0 )
    {
      ...
      {
        long texture_offset = long( act_ty / act_invz ) * surface.xscale + long( act_tx / act_invz );

        sbuffer[ offset ] = surface.picture[ texture_offset ];
...
void polygon::load( FILE *input )
{
  ...
  points[ 1 ].tx = surface.xscale-1;  points[ 1 ].ty = 0;
  points[ 2 ].tx = surface.xscale-1;  points[ 2 ].ty = surface.yscale-1;
  points[ 3 ].tx = 0;                 points[ 3 ].ty = surface.yscale-1;
}
