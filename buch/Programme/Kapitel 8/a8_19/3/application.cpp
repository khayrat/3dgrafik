...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow )
{
  ...
  message( "Steuerung:", "Pfeiltasten, Bild auf, Bild ab\tRotation der Welt\nPos 1, Ende\t\tVerschiebung der Welt\n\nR, F, D, G, E, T:\t\tRotation der aktuellen Darstellungskamera\nU, J, H, K, Z, I:\t\tVerschiebung der aktuellen Darstellungskamera\n\nEnter:\t\t\tWechsel der aktuellen Darstellungskamera\n\nEscape:\t\t\tEnde des Programms\n ");

  initialise_world( hInstance );
  pixel_32 *screen = (pixel_32 *) screen_interface.get_screen_pointer();

  window vl( 600, 450, 30, 70 );
  window vst( 300, 225, screen_x_res-300-31, screen_y_res-300-300 );
  window vsb( 300, 225, screen_x_res-300-31, screen_y_res-300-40 );

  wireframe global_view( "camera.lns" );

  wireframe user[ 2 ];
  user[ 0 ].load( "complex_system.lns" );
  user[ 1 ].load( "simple_system.lns" );
  ...
  while( 1 )
  {
    ...
    move_ls( &user[ act_user ].ls );

    vl.install();
    camera = global_view.ls;

      for( x=0 ; x<5 ; x++ ) p[ x ].display( sbuffer );
      user[ 0 ].display( sbuffer );
      user[ 1 ].display( sbuffer );
    ...
    if( act_user == 1 ) vst.display( screen, 1 );
    else vst.display( screen, 0 );
    ...
    if( act_user == 0 ) vsb.display( screen, 1 );
    else vsb.display( screen, 0 );
  }
...
uchar handle_input( matrix *m, vertex pos )
{
  ...
  if( input.event_key == VK_RETURN ) if( ++act_user > 1 ) act_user = 0;
