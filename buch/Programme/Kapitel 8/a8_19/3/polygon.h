...
class polygon
{
  ...
  public:
    void load( FILE *input, vertex *vs, long color );
...
void polygon::z_clipping( void )
{
  ...
  for( x=local_point_count-1, y=0 ; y<local_point_count ; x=y, y++ )
  {
    if( c_wpoint[ x ].wz <= z_max && c_wpoint[ y ].wz <= z_max )                               // inside
      ...
      c_wpoint[ cp_point_count++ ] = get_zplane_coords( c_wpoint[ x ], c_wpoint[ y ], z_max );
      
    else if( c_wpoint[ x ].wz > z_max && c_wpoint[ y ].wz <= z_max )                           // entering
    {
      c_wpoint[ cp_point_count++ ] = get_zplane_coords( c_wpoint[ x ], c_wpoint[ y ], z_max );
      c_wpoint[ cp_point_count++ ] = c_wpoint[ y ];	
    }	    
  }  
}
...
void polygon::project( void )
{
  for( long x=0 ; x<cp_point_count ; x++ )
  {
    ...
    spoint[ x ].sx = long( wpoint[ x ].wx / wpoint[ x ].wz *  xpr_cnst + x_res/2 );
    spoint[ x ].sy = long( wpoint[ x ].wy / wpoint[ x ].wz * -ypr_cnst + y_res/2 );
    spoint[ x ].sz = 1.0 / wpoint[ x ].wz;
...
double get_color( vector normal )
{
  ...
  if( cos_alpha < 0.0 )
    return pow( -cos_alpha, 3 ) * (used_colors.range_size - 1);
...
void polygon::load( FILE *input, vertex *vs, long color )
{
