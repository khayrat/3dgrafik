...
class wireframe
{
  ...
  public:
    ...
    void display( pixel_32 *sbuffer, pixel_32 color );
...
uchar project_point( vertex w, svertex *s )
{
  ...
  s->sx = long( w.wx * inv_z *  xpr_cnst + x_res / 2 );
  s->sy = long( w.wy * inv_z * -ypr_cnst + y_res / 2 );
  s->sz = 1.0 / w.wz;
...
void wireframe::display( pixel_32 *sbuffer, pixel_32 color )
{
  if( sbuffer == NULL || zbuffer == NULL ) exit_error( "Es wurde kein Sichtfenster definiert.\n" );
  ...
  for( long x=0 ; x<line_count ; x++ )
  {
    ...
    if( project_point( w_begin, &s_begin ) && project_point( w_end, &s_end ) )
      draw_line( s_begin, s_end, color, sbuffer );
  }
}
