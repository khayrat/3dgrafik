...
void bitmap_32::load( char *filename, pixel_32 tr, long c )
{
  load( filename );
  ...
  for( long x=0 ; x<xscale*yscale ; x++ )
  {
    ...
    switch( c )
    {
      case 0 : picture[ x ].red = picture[ x ].blue;  picture[ x ].blue = picture[ x ].green;
               break;
      case 1 : picture[ x ].green = picture[ x ].blue;  picture[ x ].blue = picture[ x ].red;
               break;
      case 3 : red_component = picture[ x ].red;  picture[ x ].red = picture[ x ].green = picture[ x ].blue;
      ...
      case 5 : picture[ x ].green = picture[ x ].blue;
               break;
      case 6 : picture[ x ].red = picture[ x ].green = picture[ x ].blue;
    }
  }
}
