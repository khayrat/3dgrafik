...
class char_block
{
  ...
  public:
    ...
    uchar display( ushort *sx, ushort sy, char c, pixel_32 color, pixel_32 *sbuffer );
...
class font
{
  ...
  public:
    ...
    void write( ushort sx, ushort sy, char *string, pixel_32 *sbuffer );

    font( char *filename, pixel_32 c );
   ~font( void ) {  if( cb ) delete [] cb;  }
} chars( "chars.txt", pixel_32( 0, 255, 0 ) );

uchar char_block::display( ushort *sx, ushort sy, char c, pixel_32 color, pixel_32 *sbuffer )
{
  if( begin_char <= c && end_char >= c )
  {
    signs[ c - begin_char ].display( *sx, sy, color, pixel_32( 255, 255, 255 ), sbuffer );
...
void font::write( ushort sx, ushort sy, char *string, pixel_32 *sbuffer )
{
  ushort delay = 0;
  ...
  for( ushort x=0 ; x<length ; x++ )
  {
    ...
    for( ushort y=0 ; y<cb_count ; y++ )
       if( cb[ y ].display( &sx, sy, string[ x ], color, sbuffer ) ) drawn = 1;
    
    if( drawn == 0 )
      cb[ cb_count-1 ].display( &sx, sy, cb[ cb_count-1 ].begin_char, color, sbuffer );
  }
}
