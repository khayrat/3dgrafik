...
class polygon
{
  private:
    ...
    long texture_count, act_texture, speed, change;
    ...
    void next_texture( void ) {  if( ++act_texture >= texture_count ) act_texture = 0;  }
  ...
  public:
    ...
    polygon( void ) : point_count( 0 ), cp_point_count( 0 ), texture_count( 0 ), 
                      act_texture( 0 ), speed( 0 ), change( 0 ), points( NULL ) { }
...
void polygon::display( vertex *vs, pixel_32 *sbuffer )
{
  ...
  change--;
  if( change <= 0 ) {  next_texture();  change = speed;  }
...
void polygon::rasterize( pixel_32 *sbuffer )
{
  ...
  long xscale = textures[ act_texture ].xscale;
  pixel_32 *picture = textures[ act_texture ].picture;
  ...
  { 
    ...
    while( length-- > 0 )
    {
      if( act_z > zbuffer[ offset ] )
      {
        ...
        long texture_offset = long( act_ty / inv_z ) * xscale + long( act_tx / inv_z );

        sbuffer[ offset ] = picture[ texture_offset ];
...
void polygon::load( FILE *input )
{
  ...
  if( (textures = new bitmap_32[ texture_count ]) == NULL )
    exit_error( "*textures: Fehler während der Reservierung von Arbeitsspeicher.\n" );
    
  speed = change = 10;

  char filename[ 100 ];  
  for( x=0 ; x<texture_count ; x++ )
  {  
    fscanf( input, "%s", filename );
    textures[ x ].load( filename );
  }
  
  if( point_count != 4 ) exit_error( "Es werden lediglich viereckige Polygone unterstützt.\n" );
  
  points[ 0 ].tx = 0;                         points[ 0 ].ty = 0;
  points[ 1 ].tx = textures[ 0 ].xscale - 1;  points[ 1 ].ty = 0;
  points[ 2 ].tx = textures[ 0 ].xscale - 1;  points[ 2 ].ty = textures[ 0 ].yscale - 1;
  points[ 3 ].tx = 0;                         points[ 3 ].ty = textures[ 0 ].yscale - 1;
}
