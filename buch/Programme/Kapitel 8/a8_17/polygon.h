...
class polygon
{
  private:
    ...
    void rasterize( primary_bitmap *surface, pixel_32 *sbuffer );

  public:
    void load( long a, long b, long c, long d, vertex *vs );
    void update_pos( matrix m ) {  normal = m * normal;  }
    void display( vertex *vs, primary_bitmap *surface, pixel_32 *sbuffer );
...
void polygon::display( vertex *vs, primary_bitmap *surface, pixel_32 *sbuffer )
{
...
double get_color( vector normal, long color_count )
{
  ...
  if( cos_alpha < 0.0 )
    return pow( mt( acos( cos_alpha ) ), 8 ) * (color_count - 1);
  
  return 0;
}

void polygon::rasterize( primary_bitmap *surface, pixel_32 *sbuffer )
{
  ...
  { 
    ...
    while( length-- > 0 )
    {
      if( act_z > zbuffer[ offset ] )
      {
        ...
        long act_bitmap = long( get_color( act_n, surface->bitmap_count ) );

        sbuffer[ offset ] = surface->bitmaps[ act_bitmap ].picture[ ty * surface->xscale + tx ];
...
void polygon::load( long a, long b, long c, long d, vertex *vs )
{
  ...
  vector u = vs[ points[ 1 ] ] - vs[ points[ 0 ] ];
  if( u.length() == 0 ) u = vs[ points[ 2 ] ] - vs[ points[ 0 ] ]; 
  vector v = vs[ points[ point_count-1 ] ] - vs[ points[ 0 ] ];
