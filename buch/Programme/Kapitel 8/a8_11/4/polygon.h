...
class polygon
{
  private:
    ...
    long texture_count, act_texture;
...
void polygon::rasterize( pixel_32 *sbuffer )
{
  ...
  long xscale, yscale, tx, ty;
  pixel_32 *picture;
  
  double borders[ 4 ] = { 12, 14, 16, 18 };
  ...
  { 
    ...
    while( length-- > 0 )
    {
      if( act_z > zbuffer[ offset ] )
      {
        ...
        act_texture = 0;
        for( x=0 ; x<texture_count-1 ; x++ ) if( z > borders[ x ] ) act_texture = x + 1;
        xscale = textures[ act_texture ].xscale;
        yscale = textures[ act_texture ].yscale;
        picture = textures[ act_texture ].picture;

        tx = long( act_tx * z * (xscale - 1) );
        tx = tx % xscale;  if( tx < 0 ) tx += xscale;

        ty = long( act_ty * z * (yscale - 1) );
        ty = ty % yscale;  if( ty < 0 ) ty += yscale;

        sbuffer[ offset ] = picture[ ty * xscale + tx ];
