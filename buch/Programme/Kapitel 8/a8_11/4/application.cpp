...
uchar handle_input( vertex pos, matrix *m )
{
  ...
  if( input.key_pressed( VK_HOME  ) ) if( (tx_translation += 0.1) > 1 ) tx_translation -= 1;
  if( input.key_pressed( VK_END   ) ) if( (tx_translation -= 0.1) < -1 ) tx_translation += 1;
  if( input.key_pressed( VK_PRIOR ) ) if( (ty_translation += 0.1) > 1 ) ty_translation -= 1;
  if( input.key_pressed( VK_NEXT  ) ) if( (ty_translation -= 0.1) < -1 ) ty_translation += 1;
