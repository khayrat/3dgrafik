...
void polygon::rasterize( pixel_32 *sbuffer )
{
  ...
  { 
    ...
    while( length-- > 0 )
    {
      ...
      {
        z = 1.0 / (act_z - clear_translation);

        act_texture = 0;
        for( x=0 ; x<texture_count-1 ; x++ ) if( z > borders[ x ] ) act_texture = x + 1;

        tx = act_tx * z * (textures[ act_texture ].xscale - 1);
        ty = act_ty * z * (textures[ act_texture ].yscale - 1);
        xscale = textures[ act_texture ].xscale;
        picture = textures[ act_texture ].picture;        

        sbuffer[ offset ] = picture[ long( ty ) * xscale + long( tx ) ];
