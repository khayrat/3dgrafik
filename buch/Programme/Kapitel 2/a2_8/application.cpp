...
{
  double wx = a * cos( alpha ) + m.wx;
  double wy = b * sin( alpha ) + m.wy;
...
{
  double wx = c * cos( alpha ) + m.wx;
  double wy = c * sin( alpha ) + m.wy;
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow )
{
  ...
  while( 1 )
  {
    ...
    draw_function( e, 0, 2*pi, pixel_32( 0, 255, 255 ), screen );
    draw_function( k, 0, 2*pi, pixel_32( 255, 255, 255 ), screen );
    draw_function( g, 0, 10, pixel_32( 0, 0, 255 ), screen );
  }
