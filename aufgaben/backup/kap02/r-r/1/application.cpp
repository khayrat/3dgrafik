...
void draw_functions(); 
void draw_function(double (* fun)(double), pixel_32); 
void draw_coordinatecross(pixel_32);
void draw_units(pixel_32);
void draw_unit(int, pixel_32, bool);
void draw_arrows();
double f(double);
double df(double);
double ddf(double);
...
void draw_function(double (*fun)(double), pixel_32 color) {
	// set color
	SDL_SetRenderDrawColor(renderer, color.red, color.green, color.blue, color.alpha);
	...
	for (double x=begin_x; x<=end_x; x+=0.01) {
		vertex point = project(x, (*fun)(x));
		// draw pixel to VRAM
		if (point.x < 0 || point.x >= SCREEN_WIDTH || point.y < 0 || point.y >= SCREEN_HEIGHT) {
			//printf("function point (%d, %d) out of sreen range.\n", point.x, point.y);
			continue;
		}	
...
void draw_functions() {
	...
	/*
	begin_x = -2*3.14; end_x=2*3.14;
	begin_y = -2; end_y=2;
	draw_function(&sin, pixel_32(0,255,0));
	draw_function(&cos, pixel_32(0,255,255));
	draw_function(&tan, pixel_32(255,255,255));
	*/
